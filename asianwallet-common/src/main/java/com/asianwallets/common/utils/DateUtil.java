...
public final class DateUtil implements Constant {
    private DateUtil() {
    }
    ...
    public static int addMinute(int i, int MINUTE) {
        Calendar c = Calendar.getInstance();
        c.setTime(hhmm.parse(MINUTE));
        c.add(Calendar.MINUTE, i);
        return hhmm.formatInt(c.getTime());
    }
    ...
    public static boolean isSameWeek(float beginDate, float endDate) {
        ...
        return isSameWeek((int) beginDate, (int) endDate);// isSameWeek(yyyymmdd.parse((int)beginDate), yyyymmdd.parse((int)endDate));
    }
    ...
    public static boolean isLegalTime() {
        ...
        String nowTime = df.format(new Date());//当前日期
        df.applyPattern("yyyy-MM-dd");
        String flag = df.format(new Date());//当前日期
        String startDate = flag.concat(" 09:00:00");//当前日期
        String endDate = flag.concat(" 09:59:59");//当前日期
        Date now = null;
        Date beginTime = null;
        Date endTime = null;
        try {
    ...
    public static boolean belongCalendar(Date nowTime, Date beginTime, Date endTime) {
        ...
        if (date.after(begin) && date.before(end)) {
            return true;//在时间段内,返回true
        } else {
            return false;//不在时间段内,返回false
        }
    }
