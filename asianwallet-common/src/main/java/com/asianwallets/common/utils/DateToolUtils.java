 ...
 * @author Wu, Hua-Zheng
 * @version v1.0.0
 * @classDesc: 功能描述: 日期工具类
 * @createTime 2018年7月9日 下午12:39:34
...
// 取值范围：就是临界点包不包含等于
    ...
    public static SimpleDateFormat sdfDateTime = new SimpleDateFormat(DateToolUtils.DATE_FORMAT_DATETIME);
    public static SimpleDateFormat sdfDateOnly = new SimpleDateFormat(DateToolUtils.DATE_FORMAT_DATEONLY);
    ...
    public static final SimpleDateFormat SHORTDATEFORMATF = new SimpleDateFormat("yyyy/MM/dd");
    public static final SimpleDateFormat SHORT_DATE_FORMAT = new SimpleDateFormat("yyyy-MM-dd");
    public static final SimpleDateFormat LONG_DATE_FORMAT = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
    public static final SimpleDateFormat LONG_DATE_A_FORMAT = new SimpleDateFormat("yyyyMMddHHmmss");
    public static final SimpleDateFormat LONG_DATE_A_FORMAT_MM = new SimpleDateFormat("yyyyMMddHH:mm");
    public static final SimpleDateFormat LONG_DATE_A_FORMAT_MMM = new SimpleDateFormat("yyyy-MM-dd HH:mm");
    ...
    public static final SimpleDateFormat LONGDATEFORMAT_S = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss");
     ...
     * yyyy/MM/dd 当前日期
     */
    public static String getReqDateF() {
        return SHORTDATEFORMATF.format(new Date());
    }

    /**
     * yyyyMMdd 当前日期
     */
    public static String getReqDateE() {
     ...
     * @return
     */
    public static String getReqDateG(Date date) {
        return LONG_DATE_FORMAT.format(date);
    }

    /**
     * yyyy-MM-dd HH:mm:ss.SSS 传入日期
     *
     * @param date
     * @return
     */
    public static String getReqDateH(Date date) {
        return LONGDATEFORMAT_SSS.format(date);
    }

    /**
     * yyyy-MM-dd HH:mm:ssaa 传入日期
     *
     * @param date
     * @return
     */
    public static String getReqDateI(Date date) {
        return LONG_DATE_FORMAT_AA.format(date);
    }
     ...
     * @return
     */
    public static String getReqDateyyyyMMddHHmmss(Date date) {
        return LONG_DATE_A_FORMAT.format(date);
    }
     ...
     * @return
     */
    public static Date getReqDateyyyyMMddHHmm(String date) {
        try {
            return LONG_DATE_A_FORMAT_MM.parse(date);
     ...
     * @return
     */
    public static Date getAssignDate(String date) {
        try {
            return LONGDATEFORMAT_S.parse(date);
        } catch (ParseException e) {
            e.printStackTrace();
            return null;
        }
     ...
     * @return
     */
    public static Date getReqDateyyyyMMddHHmmE(String date) {
        try {
            return LONG_DATE_A_FORMAT_MMM.parse(date);
        } catch (ParseException e) {
            e.printStackTrace();
            return null;
        }
    ...
    /**
     * 根据字符串获得Date  (yyyy-MM-dd HH:mm:ss)
     *
     * @param date
     * @return
     */
    public static Date getDateByE(String date) {
        try {
    ...
    public static Timestamp toSqlTimestamp(String sDate) {
        ...
        if (sDate.length() != DateToolUtils.DATE_FORMAT_DATEONLY.length()
                && sDate.length() != DateToolUtils.DATE_FORMAT_DATETIME.length()) {
            return null;
        }
        return toSqlTimestamp(sDate,
                sDate.length() == DateToolUtils.DATE_FORMAT_DATEONLY.length() ? DateToolUtils.DATE_FORMAT_DATEONLY
                        : DateToolUtils.DATE_FORMAT_DATETIME);
    ...
    public static Timestamp toSqlTimestamp(String sDate, String sFmt) {
        ...
        if (sFmt.equals(DateToolUtils.DATE_FORMAT_DATETIME)) {
            temp = sDate.replace('/', '-');
            temp = temp + ".000000000";
        } else if (sFmt.equals(DateToolUtils.DATE_FORMAT_DATEONLY)) {
            temp = sDate.replace('/', '-');
    ...
    public static String getSysDateTimeString() {
        return toString(new Date(System.currentTimeMillis()), DateToolUtils.sdfDateTime);
    }
    ...
    public static String toSqlTimestampString2(Timestamp dt) {
        ...
        String temp = toSqlTimestampString(dt, DateToolUtils.DATE_FORMAT_DATETIME);
    ...
    public static String toSqlTimestampString(Timestamp dt, String sFmt) {
        ...
        temp = dt.toString();
        if (sFmt.equals(DateToolUtils.DATE_FORMAT_DATETIME) || // "YYYY/MM/DD
// HH24:MI:SS"
                sFmt.equals(DateToolUtils.DATE_FORMAT_DATEONLY)) { // YYYY/MM/DD
            temp = temp.substring(0, sFmt.length());
            out = temp.replace('/', '-');
// }else if( sFmt.equals (DateToolUtils.DATE_FORMAT_SESSION ) ){
// //Session
// out =
...
// out += temp.substring(12,14) + temp.substring(15,17);
    ...
    public static String timestampToStringYMD(Timestamp timestamp) {
        SimpleDateFormat sdf = new SimpleDateFormat(DateToolUtils.DATE_FORMAT_DATEONLY);
    ...
    public static boolean isOverIntervalLimit(String startDateStr, String endDateStr, int interval) {
        ...
        Date endDate = null;
        try {
            startDate = DateToolUtils.parseDate(startDateStr, DateToolUtils.DATE_FORMAT_DATEONLY);
            endDate = DateToolUtils.parseDate(endDateStr, DateToolUtils.DATE_FORMAT_DATEONLY);
        } catch (Exception e) {
            log.error("[DateToolUtils异常]-{}", e);
            return false;
        }
     ...
     * @return Date
     */
    public static Date getDateFromStringTz(String src) {
        try {
            return LONGDATEFORMAT_SSZ.parse(src);
        } catch (ParseException e) {
            return null;
        }
    }
    ...
    public static Date getMaxTime(Date dt) {
        ...
        dt1 = ca.getTime();
        dt1 = DateToolUtils.getMinTime(dt1);
        ca.setTime(dt1);
    ...
    public static Date getMinTime(Date dt) {
        Date dt1 = null;
        dt1 = DateToolUtils.getDateByStr(DateToolUtils.formatDate(dt, "yyyy-MM-dd"));
        return dt1;
    }
...
// subYear==0,说明是同一年
    ...
    public static List<String> getLastDays(int countDay) {
        ...
        for (int i = 0; i < countDay; i++) {
            listDate.add(DateToolUtils.getReqDateyyyyMMdd(DateToolUtils.getDate(-i)));
        }
        return listDate;
    }
    ...
    /**
     * BigData计算 hour 小时后的时间
     *
     * @param date
     * @param hour
     * @return
     */
    public static String addNowHour(Date date, int hour) {
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(date);
        calendar.add(Calendar.HOUR, hour);
        return DateToolUtils.getDateHour(calendar.getTime()) + ":00:00";
    }
     ...
     * @param minNumber
     * @return
     * @des:20190318 RyanCai 微信过期时间有使用，修改请注意影响
     */
